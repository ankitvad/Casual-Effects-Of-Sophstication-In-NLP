...
class DecoderLayer(nn.Module):
    ...
    def forward(self, trg, enc_src, trg_mask, src_mask):
        # trg = [batch size, trg len, hid dim]
        # enc_src = ([batch size, src len, hid dim], [batch_size, hid dim] -> pooler)
        # trg_mask = [batch size, trg len]
        # src_mask = [batch size, src len]
        ...
        # encoder attention
        #         print('trg -', trg)
        #         print('enc_src -', enc_src[1].shape)
        #         print('src_mask -', src_mask)
        _trg, attention = self.encoder_attention(trg, enc_src[0], enc_src[0], src_mask)
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, input_ids, input_mask, segment_ids, trg):
        # src = [batch size, src len]
        # trg = [batch size, trg len]
        ...
        enc_src = self.bert_encoder(
            input_ids,
            attention_mask=input_mask,
            token_type_ids=segment_ids
        )

        #         enc_src = self.bert_encoder(src, src_mask)
